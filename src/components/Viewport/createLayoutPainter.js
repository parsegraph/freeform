import { Alignment, Axis, CommitLayout, Direction, paintNodeBounds, paintNodeLines } from "parsegraph";
import { BORDER_ROUNDEDNESS, BORDER_THICKNESS, BUD_SIZE, FONT_SIZE, INWARD_SEPARATION, LINE_HEIGHT, LINE_THICKNESS, nodeHasValue } from "../../settings";
...
const getNodeSize = (node, size, ctx) => {
    ...
    if (node.neighbors().hasNode(Direction.INWARD)) {
        ...
        if (node.neighbors().getAlignment(Direction.INWARD) === Alignment.INWARD_VERTICAL) {
            ...
            size[0] = Math.max(size[0], 2* BORDER_THICKNESS+ child.scale() * childSize[0]);
            ...
            size[1] = Math.max(size[1], BORDER_THICKNESS + child.scale() * childSize[1]);
    ...
    pg.siblings().forEach(node => {
        ...
        paintNodeBounds(node, (x, y, w, h) => {
            ...
            if (nodeHasValue(node) || node.neighbors().hasNode(Direction.INWARD)) {
                painter.drawBlock(x, y, w, h, BORDER_ROUNDEDNESS* scale, BORDER_THICKNESS* scale);
            } else {
...
const createLayoutPainter = (painters, bounds, glProvider, ctx, getNodeStyle) => {
    if (!getNodeStyle) {
